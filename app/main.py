...
@app.put("/admin/settings/admins/{admin_id}/password")
async def admin_change_password(admin_id: int, data: dict, current_user = Depends(require_role("admin"))):
    new_password = data.get("password")
    if not new_password or len(new_password) < 4:
        raise HTTPException(400, "Ο νέος κωδικός πρέπει να έχει τουλάχιστον 4 χαρακτήρες")
    
    # Έλεγχος ότι ο διαχειριστής υπάρχει
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
